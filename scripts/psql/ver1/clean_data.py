...
CLEANED_PATH = 'cleaned_datasets/psql/ver1'
Path(CLEANED_PATH).mkdir(exist_ok=True)
...
messages_table = messages[['message_id', 'campaign_id', 'message_type', 'client_id', 
                             'channel', 'platform', 'email_provider',
                             'sent_at', 'created_at', 'updated_at']].drop_duplicates('message_id').set_index('message_id')
logger.info("Messages table shape: %s", messages_table.shape)
messages_table.to_csv(Path(CLEANED_PATH, 'messages.csv'))
del(messages)
...
bulks.to_csv(Path(CLEANED_PATH, 'campaign_bulks.csv'))
del(bulks)
...
campaign_subjects.to_csv(Path(CLEANED_PATH, 'campaign_subjects.csv'))
...
triggers.to_csv(Path(CLEANED_PATH, 'campaign_triggers.csv'))
del(triggers)
...
unique_products.to_csv(Path(CLEANED_PATH, 'products.csv'), index=False)
...
events.to_csv(Path(CLEANED_PATH, 'events.csv'), index=False)
del(events)
...
clients.to_csv(Path(CLEANED_PATH, 'clients.csv'), index=False)
del(clients)

users = pd.concat([users, first_purchase['user_id'].drop_duplicates()]).drop_duplicates()
users.to_csv(Path(CLEANED_PATH, 'users.csv'), index=False)
del(users)
...
first_purchase.to_csv(Path(CLEANED_PATH, 'client_first_purchase_date.csv'), index=False)
del(first_purchase)
...
friends.to_csv(Path(CLEANED_PATH, 'friends.csv'), index=False)
del(friends)
